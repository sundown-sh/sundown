...
def render_html(data: ReportData, *, report_id: str = "—") -> bytes:
    ...
    for r in data.rows:
        rows.append(
            ...
            f"<td><b>{_e(r.connector)}</b><br><span style='color:#6b7280'>{_e(r.integration_name)}</span></td>"
...
def render_pdf(data: ReportData, *, report_id: str = "—") -> bytes:
    """Try WeasyPrint first; fall back to the styled HTML if unavailable."""
    html = render_html(data, report_id=report_id)
    try:
        from weasyprint import HTML

        return HTML(string=html.decode("utf-8")).write_pdf() or html
    except Exception:
        # WeasyPrint is missing/broken (most often on Windows). The HTML
        # we ship is print-styled (@page rules); the operator can print
